...
%
% Sivan Trajtenberg-Mills, Nov. 2016
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
FWHM               = 1e-9;         %Full width half maximum in the Dosseva paper
...
%build arrays
w_s_array       = linspace(w_s-3*width_in_freq_pump,w_s+3*width_in_freq_pump,N);
lambda_s_array  = 2*pi*c./w_s_array;
k_s_array       = 2*pi*signal_crystal(lambda_s_array*1e6,T)./lambda_s_array;

w_i_array       = linspace(w_i-3*width_in_freq_pump,w_i+3*width_in_freq_pump,N);
...
%plot the pump dustribution
figure(); 
imagesc((w_s_array-w_s)*1e-12, (w_i_array-w_i)*1e-12, pump);
xlabel('\Delta\omega_s [10^1^2 rad/s]');
ylabel('\Delta\omega_i [10^1^2 rad/s]');
set(gca,'YDir','normal'); colormap bone;
axis equal; title('\alpha(\omega_s,\omega_i) , the pump frequency distribution');
...
desired_beam_F         = exp(-((delta_k_array-delta_k0).^2/2/(Gaussianwidth_in_k)^2));
...
figure();
imagesc((w_s_array-w_s)*1e-12, (w_i_array-w_i)*1e-12, phase_matching_matrix);
xlabel('\Delta\omega_s [10^1^2 rad/s]');
ylabel('\Delta\omega_i [10^1^2 rad/s]');
set(gca,'YDir','normal'); colormap bone;
axis equal; title('\Phi(\omega_s,\omega_i) , the phase matching function');
set(findall(gcf,'-property','FontSize'),'FontSize',16);

figure();
imagesc((w_s_array-w_s)*1e-12, (w_i_array-w_i)*1e-12, phase_matching_matrix.*pump);
xlabel('\Delta\omega_s [10^1^2 rad/s]');
ylabel('\Delta\omega_i [10^1^2 rad/s]');
set(gca,'YDir','normal'); colormap bone;
...
for n=1:length(z)
    ...
    display(['propagating, ', num2str(n/length(z)*100), '%']);
end
